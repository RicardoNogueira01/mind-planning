...
type ColorMode = 'default' | 'parent' | 'gradient';
type ConnectionStyle = 'curved' | 'organic' | 'orthogonal-h' | 'orthogonal-v' | 'bracket';
...
export default function ConnectionsSvg({
    ...
    const getConnectionColor = (conn: Connection, fromNode?: Node): string => {
    ...
    const getNodeRect = (nodeId: string): NodeRect | null => {
    ...
    const isColorTooLight = (color: string | undefined): boolean => {
    ...
    return (
        ...
        >
            ...
            {connectionStyle === 'bracket' && (() => {
                ...
                const connectionsByParent: Record<string, Connection[]> = {};
                ...
                return Object.entries(connectionsByParent).map(([parentId, childConns]) => {
                    ...
                    return (
                        <g key={`bracket-${parentId}`}>
                            ...
                            {paths.map((pathData: { d: string; color: string }, idx: number) => (
                                <path
                            ...
                            {circles && circles.map((circle: { cx: number; cy: number; r: number; color: string }, idx: number) => (
                                <circle
            ...
            {connectionStyle !== 'bracket' && connections.map((conn) => {
                ...
                let pathData: string, labelPoint: { x: number; y: number } | undefined, start: { x: number; y: number } | undefined, end: { x: number; y: number } | undefined;
                ...
                if (connectionStyle === 'orthogonal-h') {
                    ...
                    const allNodeRects = nodes.map(n => getNodeRect(n.id)).filter((rect): rect is NodeRect => rect !== null);

                    let forceOrientation: string | undefined = undefined;
                    ...
                    const result = computeBezierPath(fromPos, toPos, {
                        ...
                        parentId: conn.from,
                        allNodeRects,
                        forceOrientation: forceOrientation as 'horizontal' | 'vertical' | undefined,
                        spreadFactor
                    });
                    pathData = result.d;
                    labelPoint = result.label;
                ...
                const strokeWidth = isHovered ? 3.5 : 3;
