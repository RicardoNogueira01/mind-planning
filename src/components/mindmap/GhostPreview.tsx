...
import React, { useState, useEffect, useCallback, useRef, useMemo, RefObject, MouseEvent as ReactMouseEvent } from 'react';
...
interface ConfirmPosition extends Position {
    screenX: number;
    screenY: number;
}
...
interface GhostPreviewProps {
    ...
    onConfirm: (position: ConfirmPosition) => void;
    onCancel: () => void;
    zoom?: number;
    pan?: Position;
...
export default function GhostPreview({
    ...
    const [showConfirmRipple, setShowConfirmRipple] = useState<boolean>(false);
    ...
    const [confirmPosition, setConfirmPosition] = useState<ConfirmPosition>({ x: 0, y: 0, screenX: 0, screenY: 0 });
    ...
    const handleMouseUp = useCallback((e: ReactMouseEvent): void => {
    ...
    useEffect(() => {
        ...
        const checkDrag = (e: globalThis.MouseEvent): void => {
    ...
    const handleConfirmPlace = useCallback((): void => {
        setIsPlacing(true);
    ...
    // Cancel popup
    const handleCancelPopup = useCallback((): void => {
        setShowConfirmPopup(false);
    }, []);
    ...
    const offsetX = smoothPos.x - (boundingBox.centerX || 0) + boundingBox.minX;
    const offsetY = smoothPos.y - (boundingBox.centerY || 0) + boundingBox.minY;
            ...
            >
                ...
                {showConfirmRipple && (
                    <div
                        style={{
                            position: 'absolute',
                            left: (boundingBox.centerX || 0) - boundingBox.minX,
                            top: (boundingBox.centerY || 0) - boundingBox.minY,
                            width: '20px',
                            height: '20px',
                            borderRadius: '50%',
                ...
                {!showConfirmPopup && (
                    <div
                        style={{
                            position: 'absolute',
                            left: (boundingBox.centerX || 0) - boundingBox.minX - 12,
                            top: (boundingBox.centerY || 0) - boundingBox.minY - 12,
                            width: '24px',
                            height: '24px',
                            pointerEvents: 'none',
