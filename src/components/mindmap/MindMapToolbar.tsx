...
const MindMapToolbar: React.FC<MindMapToolbarProps> = ({
    ...
    return (
        <div
            className={`${isMobile
                ? `fixed left-0 top-0 h-full z-[70] bg-white shadow-2xl border-r border-gray-200 w-20 p-4 transition-transform duration-300 ease-in-out ${showMobileToolbar ? 'translate-x-0' : 'translate-x-[-100%]'
                }`
            ...
            <div className={`flex ${isMobile ? 'flex-col h-full' : 'items-center'} gap-0.5`}>
                ...
                <div className={`flex ${isMobile ? 'flex-col w-full' : 'items-center'} gap-0.5`}>
                    <button
                        ...
                        className={`${isMobile ? 'w-full' : ''} p-2 rounded-lg transition-all duration-200 ${mode === 'cursor' && selectionType === 'simple'
                    ...
                    <button
                        ...
                        className={`${isMobile ? 'w-full' : ''} p-2 rounded-lg transition-all duration-200 ${mode === 'cursor' && selectionType === 'multi'
                            ? 'bg-gray-800 text-white shadow-lg'
                            : 'text-black hover:bg-gray-100'
                            }`}
                    ...
                    <button
                        ...
                        className={`${isMobile ? 'w-full' : ''} p-2 rounded-lg transition-all duration-200 ${mode === 'cursor' && selectionType === 'collaborator'
                            ? 'bg-gray-800 text-white shadow-lg'
                            : 'text-black hover:bg-gray-100'
                            }`}
                    ...
                    <button
                        onClick={() => setMode('pan')}
                        className={`${isMobile ? 'w-full' : ''} p-2 rounded-lg transition-all duration-200 ${mode === 'pan'
                            ? 'bg-gray-800 text-white shadow-lg'
                            : 'text-black hover:bg-gray-100'
                            }`}
                        title="Pan Mode"
