import React, { useMemo, useState, DragEvent, FormEvent, ChangeEvent } from 'react';
...
interface BoardViewProps {
    nodes: BoardNode[];
    onNodeUpdate?: (nodeId: string, updates: Partial<BoardNode>) => void;
}
 ...
 */
const BoardView: React.FC<BoardViewProps> = ({ nodes, onNodeUpdate }) => {
    ...
    const groupedTasks = useMemo(() => {
        const groups: Record<string, BoardNode[]> = {};
        columns.forEach(col => {
            groups[col.id] = [];
        });
    ...
    const formatDueDate = (dateString?: string): DueDateInfo | null => {
    ...
    const getProgressBarColor = (priority?: string): string => {
        switch (priority) {
            case 'high':
            case 'urgent':
                return '#EF4444';
    ...
    const handleDragStart = (e: DragEvent<HTMLDivElement>, task: BoardNode): void => {
    ...
    const handleDragEnter = (e: DragEvent<HTMLDivElement>, columnId: string): void => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const handleDrop = (e: DragEvent<HTMLDivElement>, columnId: string): void => {
        e.preventDefault();
        e.stopPropagation();
        setDraggedOverColumn(null);
    ...
    const handleCreateTask = (e: FormEvent<HTMLFormElement>): void => {
        ...
        const task: BoardNode = {
            id: taskId,
            text: newTask.text,
            priority: newTask.priority,
            status: selectedColumn || 'not-started',
                        ...
                        <form onSubmit={handleCreateTask} className="p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    required
                                    value={newTask.text}
                                    onChange={(e: ChangeEvent<HTMLInputElement>) => setNewTask({ ...newTask, text: e.target.value })}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e: ChangeEvent<HTMLInputElement>) => setNewTask({ ...newTask, assignee: e.target.value })}
