import React, { useEffect, useState, useRef, ChangeEvent, KeyboardEvent, MouseEvent, FocusEvent } from 'react';
...
const MindMapSearchBar: React.FC<MindMapSearchBarProps> = ({
    searchQuery,
    setSearchQuery,
    showSearchList,
    setShowSearchList,
    ...
    const [deleteDescendants, setDeleteDescendants] = useState<boolean>(false);
    ...
    const handleNodeClick = (node: SearchNode): void => {
        setSelectedNode(node.id);
        setShowSearchList(false);
        setPan({
            x: window.innerWidth / 2 - (node.x || 0),
            y: window.innerHeight / 2 - (node.y || 0)
        });
    };
                        ...
                        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-200 p-5">
                            ...
                            <label className="mt-3 flex items-center gap-2 text-sm text-gray-700">
                                <input
                                    ...
                                    onChange={(e: ChangeEvent<HTMLInputElement>) => setDeleteDescendants(e.target.checked)}
                                />
