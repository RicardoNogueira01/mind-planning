import React, { useState, MouseEvent, KeyboardEvent, ChangeEvent, ReactNode } from 'react';
...
const NodeCard: React.FC<NodeCardProps> = ({
    ...
    const getCollaboratorInfo = (collabId: string): Collaborator | undefined => {
    ...
    const getShapeBorderRadius = (shape: string | undefined, fallbackRadius: string): string => {
        switch (shape) {
    ...
    const calculateDaysUntilDue = (): number | null => {
    ...
    const getPriorityDisplay = (): PriorityDisplay | null => {
    ...
    const getStatusDisplay = (): StatusDisplay | null => {
        switch (node.status) {
            case 'completed':
    ...
    const calculateNodeWidth = (): number => {
    ...
    return (
        <div
            ...
            style={{
                ...
                ...(selected && { '--tw-ring-color': themeStyles.selectedRing } as React.CSSProperties),
            }}
        ...
        >
            ...
            <button
                ...
                onClick={(e: MouseEvent<HTMLButtonElement>) => {
                    e.stopPropagation();
                    onSelect?.(node.id, e as unknown as MouseEvent);
                }}
                style={{
