...
export default function HandoffChecklist({ 
  ...
  const [step, setStep] = useState(1); // 1: Select task, 2: Select person, 3: Checklist

  // Get tasks that can be handed off (assigned to current user)
  const handoffableTasks = useMemo(() => {
  ...
  const currentOwner = selectedTask ? getCollaborator(selectedTask.collaborators?.[0]) : null;
...
HandoffChecklist.propTypes = {
  nodes: PropTypes.array,
  collaborators: PropTypes.array,
  onHandoffComplete: PropTypes.func,
