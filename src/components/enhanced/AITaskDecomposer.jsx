 ...
 */
export default function AITaskDecomposer({ node, onDecompose, onClose }) {
  ...
  const analyzeTask = async () => {
    ...
    const formattedSuggestions = subtasks.map((subtask, index) => ({
  ...
  const handleAddSingle = (suggestion) => {
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <span className={`text-sm font-medium ${
                        selectedSuggestions.has(suggestion.id) ? 'text-indigo-900' : 'text-gray-700'
                      }`}>
                        {suggestion.text}
                      </span>
                      <button
...
AITaskDecomposer.propTypes = {
  node: PropTypes.shape({
    id: PropTypes.string,
    text: PropTypes.string
  }),
  onDecompose: PropTypes.func.isRequired,
