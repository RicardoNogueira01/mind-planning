...
export default function CrossProjectTimeline({ 
  ...
  const [viewStart, setViewStart] = useState(new Date('2024-12-01'));
  ...
  const dateRange = useMemo(() => {
    ...
    let daysToShow = zoomLevel === 'day' ? 14 : zoomLevel === 'week' ? 42 : 90;
  ...
  const getTaskPosition = (task) => {
    ...
    const startOffset = Math.max(0, Math.floor((start - rangeStart) / (1000 * 60 * 60 * 24)));
    const endOffset = Math.min(totalDays, Math.ceil((end - rangeStart) / (1000 * 60 * 60 * 24)));
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col mx-4 ${
      expandedView ? 'fixed inset-4 z-[90]' : 'w-full max-w-sm sm:max-w-2xl md:max-w-4xl lg:max-w-6xl max-h-[90vh]'
    }`}>
      {/* Header */}
            ...
            {['day', 'week', 'month'].map(level => (
              <button
                ...
                className={`px-2 py-1 text-xs font-medium rounded transition-colors ${
                  zoomLevel === level ? 'bg-white shadow-sm' : 'hover:bg-white/50'
                }`}
              >
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <button
            ...
            className={`px-2 py-1 text-xs rounded-lg transition-colors flex items-center gap-1 ${
              showConflicts ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'
            }`}
          >
          ...
          <div className="flex gap-1">
            {projects.map(p => (
              <button
                ...
                className={`px-2 py-1 text-xs rounded-lg transition-colors flex items-center gap-1 ${
                  hiddenProjects.has(p.id) ? 'bg-gray-100 text-gray-400' : ''
                }`}
                style={{ 
            ...
            <div className="flex">
              {dateRange.map((date, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex-shrink-0 border-r border-gray-100 text-center text-[10px] py-1 ${
                      isToday(date) ? 'bg-indigo-50' : isWeekend(date) ? 'bg-gray-50' : ''
                    }`}
...
CrossProjectTimeline.propTypes = {
