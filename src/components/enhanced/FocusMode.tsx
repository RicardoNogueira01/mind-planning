 ...
 */
export default function FocusMode({ selectedNode, onComplete, onClose }: FocusModeProps) {
    // Timer state
    ...
    // Audio ref (unused but kept for potential future use)
    ...
    useEffect(() => {
        ...
        if (isActive && !isPaused && timeLeft > 0) {
            ...
            if (mode === 'focus') {
                ...
                const breakMode: TimerMode = isLongBreak ? 'longBreak' : 'shortBreak';
    ...
    const getModeColors = (): ModeColors => {
        switch (mode) {
            case 'focus':
                return {
                    ...
                    <div className="space-y-2 pt-2 border-t border-gray-100">
                        <label className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className={`w-10 h-6 rounded-full transition-colors ${settings.autoStartBreaks ? 'bg-indigo-500' : 'bg-gray-300'
                                    }`}
                            >
                                <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${settings.autoStartBreaks ? 'translate-x-5' : 'translate-x-1'
                                    }`} />
                            </button>
                        </label>
                        ...
                        <label className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className={`w-10 h-6 rounded-full transition-colors ${settings.autoStartFocus ? 'bg-indigo-500' : 'bg-gray-300'
                                    }`}
                            >
                                <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${settings.autoStartFocus ? 'translate-x-5' : 'translate-x-1'
                                    }`} />
                            </button>
                        </label>
                    </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-1">
                        {[...Array(settings.sessionsUntilLongBreak)].map((_, i) => (
                            <div
                                key={i}
                                className={`w-2 h-2 rounded-full transition-colors ${i < (sessionsCompleted % settings.sessionsUntilLongBreak) ? 'bg-white' : 'bg-white/30'
                                    }`}
                            />
                        ))}
                    </div>
