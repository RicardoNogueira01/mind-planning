...
type ViewType = 'list' | 'create' | 'templates';
...
export default function AutomationBuilder({
    ...
    const handleToggleAutomation = (id: number): void => {
        setAutomations(prev => prev.map(a =>
            a.id === id ? { ...a, enabled: !a.enabled } : a
        ));
    };

    const handleDeleteAutomation = (id: number): void => {
        setAutomations(prev => prev.filter(a => a.id !== id));
    };

    const handleCreateAutomation = (): void => {
    ...
    const getTrigger = (id: string): Trigger | undefined => TRIGGERS.find(t => t.id === id);
    const getAction = (id: string): Action | undefined => ACTIONS.find(a => a.id === id);
    ...
    return (
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden w-full max-w-sm sm:max-w-xl md:w-[600px] max-h-[90vh] flex flex-col mx-4">
            ...
            <div className="flex border-b border-gray-200">
                {([
                    { id: 'list' as ViewType, label: 'My Automations' },
                    { id: 'create' as ViewType, label: 'Create New' },
                    { id: 'templates' as ViewType, label: 'Templates' }
                ]).map(tab => (
                    <button
                        key={tab.id}
                        onClick={() => setView(tab.id)}
                        className={`flex-1 py-2.5 text-sm font-medium transition-colors ${view === tab.id
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    {newAutomation.action === 'notify' && (
                        <div>
                            ...
                            <textarea
                                value={(newAutomation.actionConfig.message as string) || ''}
                    ...
                    {newAutomation.action === 'assign' && (
                        <div>
                            ...
                            <select
                                value={(newAutomation.actionConfig.assignee as string) || ''}
                                onChange={(e) => setNewAutomation(prev => ({
                                    ...prev,
                    ...
                    {newAutomation.action === 'set_priority' && (
                        <div>
                            ...
                            <select
                                value={(newAutomation.actionConfig.priority as string) || ''}
                                onChange={(e) => setNewAutomation(prev => ({
                                    ...prev,
