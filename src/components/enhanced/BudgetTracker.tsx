...
interface BudgetAnalysis {
    ...
    teamSpending: Record<string, number>;
    ...
    const budgetAnalysis = useMemo<BudgetAnalysis>(() => {
        ...
        nodes.forEach(node => {
            ...
            if (assignees.length === 0) {
                const rateValues = Object.values(hourlyRates);
        ...
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden w-full max-w-sm sm:max-w-md md:w-[480px] max-h-[85vh] flex flex-col mx-4">
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 border border-gray-200">
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <div className={`text-2xl font-bold ${budgetAnalysis.remaining < 0 ? 'text-red-600' : 'text-green-600'
                                }`}>
                    ...
                    <div className="relative">
                        <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-500 ${budgetAnalysis.isOverBudget
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className={`rounded-lg p-3 border ${budgetAnalysis.runway < 30
                            ? 'bg-red-50 border-red-100'
                            : 'bg-green-50 border-green-100'
                        }`}>
                        <div className="flex items-center gap-2 mb-1">
                            <Calendar className={`w-4 h-4 ${budgetAnalysis.runway < 30 ? 'text-red-600' : 'text-green-600'
                                }`} />
                            <span className={`text-xs font-medium ${budgetAnalysis.runway < 30 ? 'text-red-700' : 'text-green-700'
                                }`}>
                                Runway
                            </span>
                        </div>
                        <div className={`text-lg font-bold ${budgetAnalysis.runway < 30 ? 'text-red-900' : 'text-green-900'
                            }`}>
                    ...
                    <div className={`rounded-lg p-3 border ${budgetAnalysis.projectedOverrun > 0
                            ? 'bg-amber-50 border-amber-100'
                            : 'bg-green-50 border-green-100'
                        }`}>
                        <div className="flex items-center gap-2 mb-1">
                            <TrendingDown className={`w-4 h-4 ${budgetAnalysis.projectedOverrun > 0 ? 'text-amber-600' : 'text-green-600'
                                }`} />
                            <span className={`text-xs font-medium ${budgetAnalysis.projectedOverrun > 0 ? 'text-amber-700' : 'text-green-700'
                                }`}>
                                Variance
                            </span>
                        </div>
                        <div className={`text-lg font-bold ${budgetAnalysis.projectedOverrun > 0 ? 'text-amber-900' : 'text-green-900'
                            }`}>
