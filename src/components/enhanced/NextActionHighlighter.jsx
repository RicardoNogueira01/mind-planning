...
export default function NextActionHighlighter({ 
  ...
  const [editStatus, setEditStatus] = useState({ status: '', detail: '' });
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {Object.entries(STATUS_TYPES).filter(([k]) => k !== 'completed').map(([key, config]) => (
                        <button
                          ...
                          className={`p-2 rounded-lg text-xs font-medium border transition-colors flex items-center gap-2 ${
...
NextActionHighlighter.propTypes = {
