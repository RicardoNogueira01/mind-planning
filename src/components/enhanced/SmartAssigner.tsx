...
interface TeamMemberStats extends Collaborator {
    totalTasks: number;
...
interface ScoredCollab extends TeamMemberStats {
    score: number;
    taskType: string;
}
...
interface Suggestion {
    task: Node;
    taskType: string;
    recommended: ScoredCollab | undefined;
    alternatives: ScoredCollab[];
    confidence: number;
}
...
type TaskType = 'design' | 'development' | 'testing' | 'review' | 'planning' | 'research' | 'documentation' | 'general';
...
export default function SmartAssigner({
    ...
    const teamStats = useMemo<Record<string, TeamMemberStats>>(() => {
    ...
    const suggestions = useMemo<Suggestion[]>(() => {
        ...
        return unassignedTasks.map(task => {
            ...
            const scoredCollabs: ScoredCollab[] = Object.values(teamStats).map(collab => {
                let score = 50;
    ...
    const handleAccept = (taskId: string, collabId: string): void => {
    ...
    const handleAutoAssignAll = async (): Promise<void> => {
