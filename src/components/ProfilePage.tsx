...
interface HolidayRequest {
    ...
    endDate: string;
    days: number;
    status: 'taken' | 'pending' | 'rejected' | 'approved';
    reason?: string;
}
...
interface RecentActivity {
    id: string | number;
    type: 'completed' | 'in_progress' | 'inProgress' | 'overdue';
    task: string;
    project: string;
    time: string;
    icon: string;
...
const ProfilePage: React.FC = () => {
        ...
        const fetchProfile = async (): Promise<void> => {
            try {
                ...
                setProfile({
                    ...
                    initials: data.initials || (data.name ? data.name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) : 'U'),
                    ...
                    teams: Array.isArray(data.memberOfTeams)
                        ? data.memberOfTeams.map((t: any) => typeof t === 'string' ? t : t?.team?.name || t?.name || 'Unknown Team')
                        : [],
    ...
    const calculateHolidays = (requests: any[]): Holidays => {
