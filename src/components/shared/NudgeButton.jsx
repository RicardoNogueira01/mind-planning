...
const getNudgeStorageKey = (userId) => `nudges_${userId}_${new Date().toDateString()}`;

// Get remaining nudges for today
const getRemainingNudges = (userId, maxNudges = 5) => {
...
const incrementNudgeCount = (userId) => {
    const key = getNudgeStorageKey(userId);
    const stored = localStorage.getItem(key);
    const count = stored ? parseInt(stored, 10) : 0;
...
export function NudgeButton({
    ...
    const [remaining, setRemaining] = useState(() => getRemainingNudges(senderId, maxNudgesPerDay));
...
NudgeButton.propTypes = {
    ...
    maxNudgesPerDay: PropTypes.number,
 ...
 */
export function NudgeReceiver({ children }) {
    ...
    useEffect(() => {
        ...
        window.addEventListener('nudge-received', handleNudge);
        return () => window.removeEventListener('nudge-received', handleNudge);
    }, []);
 ...
 */
export function triggerNudge(fromName) {
 ...
 */
export function useNudge(userId, maxNudgesPerDay = 5) {
    const [remaining, setRemaining] = useState(() => getRemainingNudges(userId, maxNudgesPerDay));

    const sendNudge = useCallback((recipientId, recipientName, socket) => {
