...
import { Search, Bell, Languages, CheckCircle, AlertTriangle, Users, Calendar, FileText, X, Check, MessageCircle, Send, Maximize2, Minimize2, LucideIcon } from 'lucide-react';
...
const TopBar: React.FC<TopBarProps> = ({ showSearch = true }) => {
    ...
    const [messages] = useState<MessagesMap>({
        1: [
    ...
    useEffect(() => {
        const handleOpenChatWith = (event: CustomEvent<OpenChatEventDetail>) => {
        ...
        window.addEventListener('openChatWith', handleOpenChatWith as EventListener);
        return () => window.removeEventListener('openChatWith', handleOpenChatWith as EventListener);
    }, []);
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                {totalUnreadMessages > 0 && (
                                    <span className="absolute top-0.5 right-0.5 min-w-[16px] h-[16px] bg-red-500 rounded-full ring-2 ring-white flex items-center justify-center px-0.5">
                                        <span className="text-white text-[9px] font-bold leading-none">{totalUnreadMessages}</span>
                                    </span>
                                )}
                            </button>
                        </div>
                        ...
                        {selectedContact ? (
                            <div className="flex-1 flex flex-col bg-white">
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                    {messages[selectedContact.id as number]?.map(message => (
                                        <div
                                            key={message.id}
