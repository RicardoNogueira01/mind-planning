...
const GridColorPicker: React.FC<GridColorPickerProps> = ({
    ...
    const handleCustomColorChange = (e: ChangeEvent<HTMLInputElement>): void => {
    ...
    const handleNativeColorChange = (e: ChangeEvent<HTMLInputElement>): void => {
    ...
    const fixedPos = anchorRect ? {
        ...
        left: Math.max(8, Math.min(anchorRect.left || 0 - 80, window.innerWidth - 260 - 8)),
        top: Math.max(8, Math.min((anchorRect.bottom || 0) + 8, window.innerHeight - 300)),
        zIndex: 5000
    } : null;
