...
const Dashboard: React.FC = () => {
    const { t } = useLanguage();
    // @ts-ignore
    const { currentTheme, setCurrentTheme, themes } = useTheme() as { currentTheme: string, setCurrentTheme: (id: string) => void, themes: Theme[] };
    ...
    const getCurrentDayInfo = (): DateInfo => {
        const now = new Date();
    ...
    const [recentCompletedTasks] = useState<CompletedTask[]>([
    ...
    const [upcomingHolidays] = useState<Holiday[]>([
    ...
    const [recentMindMaps] = useState<MindMap[]>([
        {
            id: 1,
    ...
    const [teamHolidayRequests] = useState<HolidayRequest[]>([
        {
            id: 1,
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            <h2 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-gray-900 via-blue-900 to-gray-900 bg-clip-text text-transparent mb-2">
                                {themes.find((t: Theme) => t.id === currentTheme)?.greeting || t('greeting.hello')}, John Doe {themes.find((t: Theme) => t.id === currentTheme)?.emoji || '👋'}
                            </h2>
                            ...
                            <div className="relative" ref={themeMenuRef}>
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        <Palette size={18} />
                                        <span>{themes.find((t: Theme) => t.id === currentTheme)?.name || 'Theme'}</span>
                                    </div>
                                    {themes.find((t: Theme) => t.id === currentTheme)?.icon || ''}
                                </button>
