import React, { useState, useRef, useEffect, ChangeEvent, KeyboardEvent as ReactKeyboardEvent } from 'react';
import { createPortal } from 'react-dom';
import NodePopup from '../mindmap/NodePopup';
import { NotesPopupProps, MentionItem, Attachment, Collaborator } from './types';
...
export default function NotesPopup({
    ...
    onChange,
    onClose
}: NotesPopupProps): React.ReactElement | null {
    const editorRef = useRef<HTMLDivElement>(null);
    const [showAttachmentMenu, setShowAttachmentMenu] = useState<boolean>(false);
    const [attachmentMenuPosition, setAttachmentMenuPosition] = useState<MenuPosition>({ top: 0, left: 0 });
    ...
    const applyFormat = (command: string, value: string | null = null): void => {
    ...
    const handleKeyUp = (e: ReactKeyboardEvent<HTMLDivElement>): void => {
    ...
    const checkForMention = (): void => {
        ...
        const textBeforeCursor = (range.startContainer as Text).textContent?.substring(0, range.startOffset) || '';
        ...
        if (atIndex !== -1) {
            ...
            const filteredAttachments: MentionItem[] = allAttachments
                .filter((att: Attachment) => att.name.toLowerCase().includes(searchText))
                .map((att: Attachment) => ({
                    ...att,
                    type: 'attachment' as const,
                    displayType: att.type
                }));

            const filteredCollaborators: MentionItem[] = allCollaborators
                .filter((collab: Collaborator) => collab.name.toLowerCase().includes(searchText))
            ...
            const combined: MentionItem[] = [...filteredCollaborators, ...filteredAttachments];
    ...
    const insertAttachmentReference = (item: MentionItem): void => {
        const selection = window.getSelection();
    ...
    const insertList = (ordered: boolean = false): void => {
        ...
        >
            ...
            <div className="flex items-center justify-center gap-1 mb-4 pb-3 border-b border-gray-200 flex-wrap">
                ...
                <button
                    ...
                    className="w-9 h-9 flex items-center justify-center rounded hover:bg-gray-100 transition-colors border border-gray-200"
                    title="Bold"
                    type="button"
                >
                ...
                <select
                    ...
                    className="h-9 px-2 rounded border border-gray-200 hover:bg-gray-100 transition-colors text-sm text-black cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500"
                    title="Font Size"
                ...
                <button
                    ...
                    className="px-2 h-9 flex items-center justify-center rounded hover:bg-gray-100 transition-colors text-sm font-semibold text-black border border-gray-200"
                    title="Heading 1"
                    type="button"
                >
                    H1
                ...
                <button
                    ...
                    className="w-9 h-9 flex items-center justify-center rounded hover:bg-gray-100 transition-colors text-black border border-gray-200"
                    title="Bullet List"
                    type="button"
                >
