...
interface RoundColorPickerProps {
    currentColor: string;
...
const RoundColorPicker: React.FC<RoundColorPickerProps> = ({
    currentColor,
    onColorSelect,
    onClose,
    ...
    const getColorPosition = (ringIndex: number, colorIndex: number, totalColors: number): ColorPosition => {
    ...
    // Handle custom color
    const handleCustomColorApply = (): void => {
    ...
    const fixedPos = anchorRect ? {
        ...
        left: Math.max(8, Math.min((anchorRect.left || 0) - 20, window.innerWidth - 280 - 8)),
        top: Math.max(8, (anchorRect.bottom || 0) + 8),
        zIndex: 5000
    } : null;
                ...
                {colorRings.map((ring, ringIndex) => (
                    <div key={`ring-${ring[0]}-${ring.length}`} className="absolute inset-0">
                        {ring.map((color, colorIndex) => {
                            const colorPosition = getColorPosition(ringIndex, colorIndex, ring.length);
                            const isSelected = selectedColor === color;

                            return (
                                <button
                                    key={color}
                                    className={`absolute w-6 h-6 rounded-full border-2 transition-all duration-200 hover:scale-125 hover:shadow-lg ${isSelected
                                        ? 'border-gray-800 shadow-lg scale-110 ring-2 ring-blue-500 ring-offset-2'
                                        : 'border-white hover:border-gray-300'
                                        }`}
                                    style={{
                                        backgroundColor: color,
                                        left: `${colorPosition.x - 12}px`,
                                        top: `${colorPosition.y - 12}px`,
